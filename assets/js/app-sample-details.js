...
(function () {
  ...
  if (!permissionService.initialized) {
    ...
    notificationService.show(
      "Không có quyền truy cập. Vui lòng kiểm tra lại URL.",
      "error"
    );
    return;
  }
  ...
  const TRANG_THAI_TONG_HOP = [
    ...
    },
    {
      ...
      applyFor: ["NOI_BO"],
      description:
        "Sau khi mã hóa (có ma_mau), chờ chuyển mẫu cho nhân viên phân tích - CHỈ DÀNH CHO MẪU NỘI BỘ",
    },
    ...
    // === TRẠNG THÁI MẪU BÊN NGOÀI (THẦU) - 2 trạng thái ===
    {
      ...
      applyFor: ["BEN_NGOAI"],
      description:
        "Sau khi mã hóa, chờ duyệt thầu - CHỈ DÀNH CHO MẪU BÊN NGOÀI",
    },
    {
      ...
      applyFor: ["BEN_NGOAI"],
      description:
        "Sau khi duyệt thầu, chờ gửi mẫu - CHỈ DÀNH CHO MẪU BÊN NGOÀI",
    },
  ...
  const BULK_ACTIONS_CONFIG = {
    ...
    CHO_MA_HOA: {
      allowedActions: [],
      description:
        "Tự động chuyển sang CHO_CHUYEN_MAU (nội bộ) hoặc CHO_DUYET_THAU (bên ngoài) sau khi có ma_mau",
    },
    ...
    all: {
      ...
      allowedActions: [],
      description:
        "Hiển thị tất cả bulk actions khi không lọc theo trạng thái cụ thể",
    },
  };
  ...
  const BULK_ACTION_STATUS_TRANSITIONS = {
    ...
    updateResult: {
      ...
      description:
        "Cập nhật kết quả: DANG_PHAN_TICH/PHAN_TICH_LAI → CHO_DUYET_KQ",
    },
    approve: {
      ...
      description:
        "Phê duyệt: CHO_DUYET_KQ → HOAN_THANH (nếu đạt) hoặc PHAN_TICH_LAI (nếu không đạt)",
    },
    reanalyzed: {
    ...
    try {
      ...
      if (response && response.data) {
        ...
        if (chiTietMauData.length === 0) {
          ...
          notificationService.show(
            `Tìm thấy ${paginationState.totalRecords} kết quả`,
            "success"
          );
        }
      }
  ...
  function reorderColumnsArray(columnsArray) {
    ...
    try {
      ...
      if (savedOrder.length !== columnsArray.length) {
        console.warn(
          `⚠️ Số lượng cột không khớp: saved=${savedOrder.length}, actual=${columnsArray.length}`
        );
  ...
  function renderColumnsList() {
    ...
    if (!chiTietMauTable) {
      container.html(
        '<div class="alert alert-warning">Chưa khởi tạo DataTable</div>'
      );
      return;
    }
    ...
    console.log(
      "📊 [renderColumnsList] Saved order in localStorage:",
      columnSettings.order
    );
    ...
    console.log(
      "�️ [renderColumnsList] Current → Original map:",
      currentToOriginalMap
    );
  ...
  function saveColumnSettingsFromModal() {
    ...
    console.log(
      "💾 [saveColumnSettings] Starting to collect column settings..."
    );
    console.log(
      "💾 [saveColumnSettings] Total column items:",
      columnItems.length
    );
    ...
    columnItems.each(function (domPosition) {
      ...
      console.log(
        `  [DOM ${domPosition}] Current: ${currentIndex}, Original: ${originalIndex}, Visible: ${isVisible}`
      );
    ...
    console.log(
      "💾 [saveColumnSettings] New column order (original indexes):",
      newOrder
    );
    console.log(
      "💾 [saveColumnSettings] New visibility settings:",
      newVisibility
    );

    // ⭐ SO SÁNH với settings cũ
    const hasOrderChanged =
      JSON.stringify(columnSettings.order) !== JSON.stringify(newOrder);
    const hasVisibilityChanged =
      JSON.stringify(columnSettings.visibility) !==
      JSON.stringify(newVisibility);

    console.log("💾 [saveColumnSettings] Order changed:", hasOrderChanged);
    console.log(
      "💾 [saveColumnSettings] Visibility changed:",
    ...
    if (saveColumnSettings()) {
      ...
      if (hasOrderChanged) {
        console.log(
          "🔄 [saveColumnSettings] Order changed, reinitializing DataTable..."
        );
        reinitDataTableWithNewSettings();
        notificationService.show(
          "Đã lưu và áp dụng cài đặt cột! (Bảng đã được làm mới)",
          "success"
        );
      } else if (hasVisibilityChanged) {
        console.log(
          "👁️ [saveColumnSettings] Only visibility changed, applying directly..."
        );
        applyColumnSettings();
  ...
  function reinitDataTableWithNewSettings() {
    ...
    try {
      ...
      notificationService.show(
        "Có lỗi khi áp dụng cài đặt. Vui lòng reload trang.",
        "error"
      );
    }
  }
  ...
  window.debugColumnSettings = function () {
    ...
    console.log(
      "1. ColReorder Extension Available:",
  ...
  function updateProgressStats() {
    if (!chiTietMauData || chiTietMauData.length === 0) {
      console.warn(
        "⚠️ Không có dữ liệu để thống kê, các bảng dữ liệu đang rỗng"
      );
      return;
    }
  ...
  function updateLoadMoreButton() {
    ...
    if ($loadMoreBtn.length) {
      if (
        ...
        $loadMoreBtn
          .prop("disabled", true)
          .html('<i class="ri-check-line me-2"></i>Đã tải hết dữ liệu');
      } else {
    ...
    scrollContainer.on("scroll", function () {
      ...
      if (scrollTop + clientHeight >= scrollHeight - 50) {
        ...
        if (
          ...
          console.log(
            "🔄 Trigger load more: At last page and scrolled to bottom"
          );
  ...
  function bindProgressFilterEvents() {
    ...
    console.log(
      "✅ Đã gắn kết sự kiện filter tiến độ (13 trạng thái tổng hợp)"
    );
  }
    ...
    if (isGroupingEnabled && selectedGroupColumns.length > 0) {
      ...
      tableConfig.rowGroup = {
        ...
        startRender: function (rows, group, level = 0) {
          ...
          let currentColumn =
            selectedGroupColumns[level] || selectedGroupColumns[0];
          let label = columnLabels[currentColumn] || currentColumn;

          // Xử lý giá trị null/undefined
          let displayGroup =
            group || '<em class="text-muted">Chưa có dữ liệu</em>';
      ...
      const columnIndex = sampleDetailsTableService.getColumnIndexByValue(
      ...
      const defaultColumnIndex =
        sampleDetailsTableService.getColumnIndexByValue(
          GROUP_BY_COLUMNS_CONFIG,
    ...
    tableConfig.columns = [
      ...
      },
      {
        ...
        render: function (data, type, row) {
          ...
          hanHoanThanh = hanHoanThanh
            ? DateFormatter.toVietnamese(hanHoanThanh)
            : "";
      ...
      },
      {
        ...
        render: function (data, type, row) {
          ...
          return tienTo
            ? `<span class="badge bg-label-info">${tienTo}</span>`
            : "";
        },
      },
      {
      ...
      },
      {
        ...
        render: function (data, type, row) {
          ...
          if (nguoiDuyet && thoiGianDuyet) {
            tooltipContent =
              `Phê duyệt bởi: ${nguoiDuyet}\nThời gian: ${thoiGianDuyet}`.replace(
                /"/g,
                "&quot;"
              );
          } else {
      ...
      },
      {
        ...
        render: function (data, type, row) {
          ...
          const isCompleted =
            row.trang_thai_phan_tich === "9.Hoàn thành" ||
            row.trang_thai_phan_tich === "10.Hủy";
      ...
      },
      {
        ...
        render: function (data, type, row) {
          ...
          if (data) {
            ...
            const tooltipContent = historyLines
              .join("\n")
              .replace(/"/g, "&quot;");
            html = `<div data-bs-toggle="tooltip" data-bs-placement="left" title="${tooltipContent}">${html}</div>`;
          }
  ...
  function bindEvents() {
    ...
    $(document).on(
      "input",
      "#formDonGia, #formChietKhau",
      calcByFormulaService.calcThanhTien
    );
  ...
  function updateBulkActionsToolbar() {
    ...
    const config =
      BULK_ACTIONS_CONFIG[currentStatusFilter] || BULK_ACTIONS_CONFIG.all;
  ...
  function handleAddNew() {
    ...
    setFormMode("add");
    $("#chiTietMauModalTitle").html(
      '<i class="icon-base ri ri-add-line me-2"></i>Thêm chi tiết mẫu mới'
    );
  ...
  function handleEdit() {
    ...
    if (rowData) {
      ...
      setFormMode("edit");
      $("#chiTietMauModalTitle").html(
        '<i class="icon-base ri ri-edit-box-line me-2"></i>Chỉnh sửa chi tiết mẫu'
      );
      elements.modal.modal("show");
    }
  }
  ...
  function handleView() {
    ...
    if (rowData) {
      ...
      setFormMode("view");
      $("#chiTietMauModalTitle").html(
        '<i class="icon-base ri ri-eye-line me-2"></i>Chi tiết mẫu'
      );
      elements.modal.modal("show");
    }
  }
  ...
  function setFormMode(mode) {
    ...
    const title =
      mode === "add"
        ...
        : "Xem chi tiết";
    $("#chiTietMauModalTitle").html(
      `<i class="ri-file-line me-2"></i>${title} chi tiết mẫu`
    );
  }
  ...
  async function updateStatus(updateData) {
    try {
      ...
      console.error(
        "❌ Lỗi ở hàm updateStatus xảy ra khi update cho id",
        id,
        ":",
        error.message
      );
  ...
  async function bulkCreateRecord(dataArray) {
    try {
      ...
      notificationService.show(
        "Thêm mới hàng loạt thất bại: " + error.message,
        "error"
      );
    }
  }
  ...
  async function bulkUpdateRecord(updates) {
    try {
      ...
      updatedData.forEach((updatedItem) => {
        const index = chiTietMauData.findIndex(
      ...
      notificationService.show(
        "Cập nhật hàng loạt thất bại: " + error.message,
        "error"
      );
    }
  }
  ...
  function updateTableRowInPlace(updatedItems) {
    ...
    updatedItems.forEach((updatedItem, index) => {
      ...
      const dataIndex = chiTietMauData.findIndex(
        (item) => item.id === updatedItem.id
      );

      if (dataIndex === -1) {
        console.error(
          `❌ [UPDATE TABLE] Data index not found for ID: ${updatedItem.id}`
        );
        return;
      }
      ...
      if (targetRowIndex !== -1) {
        ...
        console.error(
          `❌ [UPDATE TABLE] Row index not found for ID: ${updatedItem.id}`
        );
      }
    });
  ...
  async function executeBulkUpdateStatus(
    selectedItems,
    crrStatus,
    showModalAndHandleUpdate
  ) {
    if (!Array.isArray(selectedItems) || selectedItems.length === 0) {
      notificationService.show(
        "Vui lòng chọn ít nhất một mục để cập nhật trạng thái",
        "warning"
      );
      return;
    }
    ...
    const validItems = selectedItems.filter(
      (item) => item.trang_thai_tong_hop === crrStatus
    );
  ...
  function handleStatusUpdateSuccess(validItems, updatedCount) {
    try {
      ...
      const updatedItems = validItems
        .map((item) => {
          const originalItem = chiTietMauData.find(
      ...
      throw new Error(
        "Lỗi khi xử lý sau cập nhật trạng thái: " + error.message
      );
    }
  }
  ...
  async function executeBulkReceiveTarget(selectedItems) {
    ...
    const validItems = selectedItems.filter(
      (item) => item.trang_thai_tong_hop === "CHO_CHUYEN_MAU"
    );
    const invalidItems = selectedItems.filter(
      (item) => item.trang_thai_tong_hop !== "CHO_CHUYEN_MAU"
    );
    ...
    const result = await Swal.fire({
      ...
      html: `
        <div class="text-start">
          <p>Bạn xác nhận nhận <strong>${
            validItems.length
          }</strong> mẫu phân tích?</p>
    ...
    if (result.isConfirmed) {
      ...
      try {
        ...
        console.log(
          `✅ Bulk receive completed: ${updatedCount} items updated, ${updatedRowsCount} rows highlighted`
        );
      } catch (error) {
        console.error("❌ Lỗi khi nhận chỉ tiêu:", error);
        notificationService.show(
          "Có lỗi xảy ra khi nhận chỉ tiêu: " + error.message,
          "error"
        );
      } finally {
        showLoading(false);
  ...
  async function executeBulkApproveThauV1(validItems) {
    ...
    partners.forEach((partner, index) => {
      optionHtml += `<option ${index == 0 ? "selected " : ""}value="${
        partner.name
      }">${partner.name}</option>`;
    });
    ...
    const result = await Swal.fire({
      ...
      html: `
        <div class="text-start">
          <p>Bạn xác nhận duyệt thầu cho <strong>${
            validItems.length
          }</strong> mẫu?</p>
          <div class="alert alert-info">
            <h6 class="mb-2">📋 Chuyển trạng thái:</h6>
          ...
          <div class="mb-3">
            ...
            <input type="date" id="sendDate" class="form-control" value="${
              new Date().toISOString().split("T")[0]
            }" />
          </div>
          <div class="mb-3">
      ...
      preConfirm: () => {
        ...
        const contractor = document
          .getElementById("contractorSelect")
          .value.trim();
  ...
  async function saveBulkUpdateContractor() {
    try {
      ...
      showLoading(false);
      notificationService.show(
        "Có lỗi xảy ra khi duyệt thầu: " + error.message,
        "error"
      );
    } finally {
      showLoading(false);
  ...
  async function saveBulkUpdateResult() {
    try {
      ...
      const updatePromises = $(".result-input").map(async function () {
        ...
        const ketQuaInPhieu = $(`.result-display[data-id="${itemId}"]`)
          .val()
          .trim();
        ...
        if (
          item.trang_thai_tong_hop === "DANG_PHAN_TICH" ||
          item.trang_thai_tong_hop === "PHAN_TICH_LAI"
        ) {
      ...
      showLoading(false);
      notificationService.show(
        "Có lỗi xảy ra khi lưu kết quả: " + error.message,
        "error"
      );
    } finally {
      showLoading(false);
  ...
  async function executeBulkApproveResult(validItems) {
    const result = await Swal.fire({
      ...
      html: `
        <div class="text-start">
          <p class="mb-3">Bạn xác nhận duyệt <strong>${
            validItems.length
          }</strong> mẫu?</p>
          <div class="alert alert-info">
            <h6 class="mb-2">📋 Chuyển trạng thái:</h6>
      ...
      preConfirm: () => {
        ...
        const approverName = document
          .getElementById("approverName")
          .value.trim();
    ...
    if (result.isConfirmed) {
      const { approvalDecision, approveDate, note, approverName } =
        result.value;

      try {
        showLoading(true);
        const summaryStatus =
          approvalDecision === "DAT" ? "HOAN_THANH" : "PHAN_TICH_LAI";
        const analysisStatus =
          approvalDecision === "DAT" ? "Đã hoàn thành" : "Chờ phân tích lại";

        const pheDuyetText =
          approvalDecision === "DAT" ? "1.Đạt" : "2.Không đạt";
        ...
        const updatePromises = validItems.map(async (item) => {
          ...
          originalItem.ngay_hoan_thanh_pt_gm =
            approvalDecision === "DAT" ? approveDate : "";
        ...
        notificationService.show(
          "Có lỗi xảy ra khi duyệt kết quả: " + error.message,
          "error"
        );
      } finally {
        showLoading(false);
  ...
  function validateBulkEditForm() {
    ...
    rows.forEach((row, index) => {
      ...
      const tenChiTieu = row
        .querySelector('[data-field="ten_chi_tieu"]')
        .value.trim();
      ...
      const ketQuaThucTe = row
        .querySelector('[data-field="ket_qua_thuc_te"]')
        .value.trim();
      if (ketQuaThucTe && isNaN(Number(ketQuaThucTe))) {
        // Chỉ cảnh báo nếu không phải số (có thể có kết quả dạng text)
        console.warn(
          `Dòng ${index + 1}: Kết quả thực tế không phải số - ${ketQuaThucTe}`
        );
      }
    });
    ...
    if (errors.length > 0) {
      Swal.fire({
        ...
        html: `<ul class="text-start">${errors
          .map((err) => `<li>${err}</li>`)
          .join("")}</ul>`,
        icon: "warning",
   ...
   */
  async function loadDanhSachChiTieuPaginated(
    page = 1,
    pageSize = 50,
    additionalFilters = {}
  ) {
    try {
      ...
      response.data = permissionService.filterData(
  ...
  function renderFilterHanHoanThanh() {
    ...
    if (permissionService.userParams.ngay_ket_thuc) {
      ngayKetThuc = DateFormatter.toISO(
        permissionService.userParams.ngay_ket_thuc
      );
    }
    ...
    if (permissionService.userParams.ngay_bat_dau) {
      ngayBatDau = DateFormatter.toISO(
        permissionService.userParams.ngay_bat_dau
      );
    }
  ...
  async function initializeApp() {
    ...
    try {
      // ⭐ THÊM: Load column settings từ localStorage
      loadColumnSettings();
