...
(function () {
  ...
  async function searchData(keyword) {
    try {
      ...
      if (response && response.data) {
        ...
        console.log(
          `✅ Found ${chiTietMauData.length} records for "${keyword}"`
        );
  ...
  function loadColumnSettings() {
    try {
      ...
      if (saved) {
        ...
        console.log(
          "✅ Đã load column settings từ localStorage:",
          columnSettings
        );
        return true;
      }
  ...
  function renderColumnsList() {
    ...
    const currentOrder =
      columnSettings.order && columnSettings.order.length > 0
        ? columnSettings.order
    ...
    currentOrder.forEach((colIndex, position) => {
      ...
      const itemHtml = `
        <div class="column-item list-group-item ${
          isFixed ? "disabled" : ""
        }" data-index="${colIndex}">
  ...
  function updateTableRowInPlace(updatedItems) {
    ...
    updatedItems.forEach((updatedItem, index) => {
      console.log(
        `🔍 [UPDATE TABLE] Processing item ${index + 1}/${
          updatedItems.length
        }:`,
        updatedItem.id
      );
      ...
      console.log(
        `📍 [UPDATE TABLE] Data index for ${updatedItem.id}:`,
        dataIndex
      );
      ...
      chiTietMauTable.rows().every(function (index) {
        ...
        if (rowData && rowData.id === updatedItem.id) {
          ...
          console.log(
            `🎯 [UPDATE TABLE] Found row index ${targetRowIndex} for ID: ${updatedItem.id}`
          );
      ...
      if (targetRowIndex !== -1) {
        console.log(
          `🔄 [UPDATE TABLE] Updating row ${targetRowIndex} with data:`,
          {
            id: updatedItem.id,
        ...
        console.log(
          `✅ [UPDATE TABLE] Row data updated for index ${targetRowIndex}`
        );
    ...
    console.log(
      "🎨 [UPDATE TABLE] Redrawing table and highlighting rows:",
    ...
    setTimeout(() => {
      ...
      rowsToHighlight.forEach((rowNode, index) => {
        ...
        console.log(
          `💡 [UPDATE TABLE] Highlighted row ${index + 1}/${
            rowsToHighlight.length
          }`
        );
        ...
        setTimeout(() => {
          ...
          console.log(
            `💭 [UPDATE TABLE] Removed highlight from row ${index + 1}`
          );
        }, 3000);
      });
    }, 100);
    ...
    console.log(
      "🏁 [UPDATE TABLE] COMPLETED: Updated",
      rowsToHighlight.length,
      "rows"
    );
